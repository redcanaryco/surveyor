...
class CortexXDR(Product):
  ...
  def _authenticate(self):
    ...
    if 'auth_type' in section:
      if section['auth_type'].lower() in ['standard','advanced']:
        ...
        raise ValueError(f'Cortex XDR configuration invalid, ensure "auth_type" is one of ["standard","advanced"]')
    ...
    if 'reply' not in resp:
      raise ValueError(f'Cortex could not validate credentials. Please check your configuration file and try again.')
    else:
  ...
  def _get_default_header(self):
    ...
    if self._auth_type == 'advanced':
      ...
      return {"x-xdr-timestamp": str(timestamp), "x-xdr-nonce": nonce, "x-xdr-auth-id": str(self._api_key_id),"Authorization": api_key_hash} 
    else:
      return {"Authorization": f"{self._api_key}", "x-xdr-auth-id": f"{self._api_key_id}", "Content-Type": "application/json"}
    ...
    try:
      while True:
        ...
        if data['status'] == 'SUCCESS':
          self.log.debug(f'Query ID {query_id} completed at cost {data["query_cost"]} with remaining quota {data["remaining_quota"]}')
    ...
    for tag, queries in self._queries.items():
      for query in queries:
        ...
        for event in events:
          ...
          username = event['action_process_username'] if 'action_process_username' in event else event['actor_primary_username']
          path = event['action_process_image_path'] if 'action_process_image_path' in event else event['actor_process_image_path']
          commandline = event['action_process_command_line'] if 'action_process_command_line' in event else event['actor_process_command_line']
